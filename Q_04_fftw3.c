...
{
	int sampl_pts = 512, i ;
	double x , x_max = 10, x_min = -10 , sampl_rate = (x_max - x_min)/(sampl_pts - 1) ;

	// defining array to store the x_values and analytic y_values and frequencies
	double x_arr[sampl_pts] ,  k_arr[sampl_pts]; 
	
	fftw_complex space_sampl[sampl_pts] , freq_sampl[sampl_pts] ;
	fftw_plan plan ;

	// constructing the array of space_sampl and analytic soln. sample
	x = x_min ;
	i = 0 ;
	while (x <= x_max)
	{
